...
def create_comparison(reference: dict[str, Any], difference: dict[str, Any]) -> dict[str, Any]:
    ...
    for control_id in sorted(set(before) | set(after)):
        ...
        if left is None: change = "AddedControl"
        elif right is None: change = "RemovedControl"
        else:
            ...
            elif result_state_fingerprint(left) != result_state_fingerprint(right): change = "Changed"
    ...
    return {
        ...
        "summary":{"ScoreDelta":round(diff_score-ref_score,1),"NewFindings":counts["NewFinding"],"Resolved":counts["Resolved"],"Changed":counts["Changed"],"AddedControls":counts["AddedControl"],"RemovedControls":counts["RemovedControl"],"Unchanged":counts["Unchanged"]},
        "changes":changes,
    }
...
def drift_markdown(comparison: dict[str, Any]) -> str:
    ...
    lines=["# Hardening Lens drift report","",f"* **Host:** `{comparison['computerName']}`",f"* **Baseline:** `{comparison['baseline']}`",f"* **Reference:** `{comparison['referenceScan']['Id']}` ({comparison['referenceScan']['CollectedAt']})",f"* **Current:** `{comparison['differenceScan']['Id']}` ({comparison['differenceScan']['CollectedAt']})",f"* **Score delta:** {s['ScoreDelta']:+.1f} points","","| New findings | Resolved | Changed | Added controls | Removed controls |","|---:|---:|---:|---:|---:|",f"| {s['NewFindings']} | {s['Resolved']} | {s['Changed']} | {s['AddedControls']} | {s['RemovedControls']} |","","## Changes","","| Type | Control | Severity | Before | After |","|---|---|---|---|---|"]
    ...
    for item in relevant:
        ...
        lines.append(f"| {item['ChangeType']} | `{item['ControlId']}` - {title} | {item['Severity']} | {item['BeforeStatus'] or ''} | {item['AfterStatus'] or ''} |")
...
def main() -> None:
    ...
    current=create_result("8fc7045c-a95e-4f23-bdb7-2bb858322c52",COLLECTED_AT,current_overrides)
    ...
    reference=create_result("e747bf2b-e9de-4e5a-bffe-514e51b86fe8","2026-06-12T09:38:02.1200000Z",reference_overrides)
