...
function ConvertTo-HLHtmlReport {
    ...
    [void]$builder.AppendLine('<meta http-equiv="Content-Security-Policy" content="default-src ''none''; style-src ''unsafe-inline''; script-src ''unsafe-inline''; img-src data:; base-uri ''none''; form-action ''none''">')
...
'@)
    ...
    foreach ($finding in $findings) {
        ...
        if (@($finding.references).Count -gt 0) {
            ...
            foreach ($reference in @($finding.references)) {
                $encoded = ConvertTo-HLHtmlEncoded $reference
    ...
    foreach ($result in @($ScanResult.results)) {
        ...
        $severityClass = 'sev-' + ([string]$result.severity).ToLowerInvariant()
